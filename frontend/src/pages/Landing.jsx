    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans selection:bg-indigo-100 selection:text-indigo-900">
      ...
      <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-2 text-indigo-600 font-bold text-xl tracking-tight no-underline">
            ...
            {isAuthenticated ? (
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-slate-100 border border-slate-200 text-xs font-medium text-slate-700">
                  ...
                  <span className="max-w-[110px] truncate">{user?.name || 'User'}</span>
                </div>
                <Link to="/dashboard" className="btn btn-primary shadow-sm text-xs sm:text-sm px-3 sm:px-4 py-1.5 sm:py-2 no-underline flex items-center gap-1.5">
                  <LayoutDashboard size={15} />
                  <span>Dashboard</span>
                </Link>
                <Link to="/login" className="text-xs sm:text-sm font-semibold text-slate-600 hover:text-indigo-600 transition-colors no-underline px-2 py-1">
                  Log in
                </Link>
                <Link to="/register" className="text-xs sm:text-sm font-semibold text-slate-600 hover:text-indigo-600 transition-colors no-underline px-2 py-1 hidden sm:inline-block">
                  Register
                </Link>
                <button
                  onClick={logout}
                  className="flex items-center gap-1 text-xs sm:text-sm font-semibold text-slate-500 hover:text-red-600 px-2 sm:px-2.5 py-1.5 rounded-md hover:bg-red-50 transition-colors cursor-pointer bg-transparent border-none"
              ...
              <div className="flex items-center gap-2 sm:gap-3">
                <Link
                  to="/login"
                  className="flex items-center gap-1.5 text-sm font-semibold text-slate-700 hover:text-indigo-600 px-3 py-2 rounded-lg hover:bg-slate-100 transition-colors no-underline"
                >
                  <LogIn size={17} />
                  <span>Log in</span>
                </Link>
                <Link
                  to="/register"
                  className="btn btn-primary shadow-sm text-sm px-4 py-2 no-underline flex items-center gap-1.5"
                >
                  <UserPlus size={17} />
                  <span>Register</span>
                </Link>
      ...
      {/* Hero */}
      <section className="relative overflow-hidden pt-16 pb-32 lg:pt-28 lg:pb-36">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          {/* Isolation & Active Status Notice */}
          {isAuthenticated ? (
            <div className="max-w-xl mx-auto mb-6 p-3 sm:p-4 rounded-2xl bg-indigo-50/90 border border-indigo-200 backdrop-blur-sm shadow-sm animate-[fadeIn_0.5s_ease-out]">
              <div className="flex items-center justify-center gap-2 text-sm text-slate-800 font-medium mb-2">
                <UserCheck size={18} className="text-emerald-600" />
                <span>Logged in as <strong>{user?.name}</strong> <span className="text-slate-500 font-normal">({user?.email})</span></span>
              </div>
              <div className="flex flex-wrap items-center justify-center gap-3 text-xs">
                ...
                <Link to="/login" className="font-medium text-slate-600 hover:text-indigo-600 no-underline">
                  Switch User / Sign In
                </Link>
                <span className="text-slate-300">|</span>
                <Link to="/register" className="font-medium text-slate-600 hover:text-indigo-600 no-underline">
                  Create New Account
                </Link>
                <span className="text-slate-300">|</span>
                <button onClick={logout} className="font-medium text-red-600 hover:text-red-700 bg-transparent border-none cursor-pointer p-0">
                  Log Out
                </button>
              </div>
            </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 animate-[fadeIn_1.1s_ease-out]">
            {isAuthenticated ? (
              <>
                <Link to="/setup" className="btn btn-primary shadow-lg shadow-indigo-500/30 text-base sm:text-lg px-7 py-3 rounded-xl group no-underline flex items-center gap-2">
                  <span>Start Practice Session</span>
                  <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" />
                </Link>
                <Link to="/dashboard" className="btn bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 hover:border-slate-400 text-base sm:text-lg px-7 py-3 rounded-xl shadow-sm no-underline flex items-center gap-2">
                  <LayoutDashboard size={20} />
                  <span>Go to Dashboard</span>
                </Link>
                <button
                  onClick={logout}
                  className="btn bg-slate-100 hover:bg-red-50 text-slate-600 hover:text-red-600 border border-slate-200 hover:border-red-200 text-base sm:text-lg px-6 py-3 rounded-xl shadow-sm cursor-pointer flex items-center gap-2 transition-colors"
                >
                  <LogOut size={18} />
                  <span>Log Out</span>
                </button>
              ...
              <>
                <Link to="/register" className="btn btn-primary shadow-lg shadow-indigo-500/30 text-base sm:text-lg px-8 py-3.5 rounded-xl group no-underline flex items-center gap-2">
                  <UserPlus size={20} />
                  <span>Get Started / Register</span>
                  <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" />
                </Link>
                <Link to="/login" className="btn bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 hover:border-slate-400 text-base sm:text-lg px-8 py-3.5 rounded-xl shadow-sm no-underline flex items-center gap-2">
                  <LogIn size={20} />
      ...
      <footer className="bg-white border-t border-slate-200 mt-auto">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex flex-wrap gap-6 text-sm font-medium text-slate-500">
            <Link to="/login" className="hover:text-indigo-600 transition-colors no-underline">Log In</Link>
            <Link to="/register" className="hover:text-indigo-600 transition-colors no-underline">Register</Link>
            {isAuthenticated && (
              <Link to="/dashboard" className="hover:text-indigo-600 transition-colors no-underline">Dashboard</Link>
            )}
