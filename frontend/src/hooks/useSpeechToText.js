...
export default function useSpeechToText(onTranscriptComplete) {
  ...
  const sendAudioToBackend = async (blob, isFinal, segmentId, sessionId) => {
    ...
    // Use standard generic extension; backend ffmpeg sniffs the actual format
    formData.append('file', blob, 'audio.tmp');
    ...
    try {
      ...
      if (isFinal) {
        ...
        setError("Failed to transcribe audio segment.");
        if (statusRef.current === 'transcribing') {
          setStatus('inactive');
        }
      }
  ...
  const setupMediaRecorder = useCallback((stream) => {
    ...
    mediaRecorder.ondataavailable = (e) => {
      ...
      // If actively speaking, progressively send the growing buffer for the current phrase
      // This bounded buffer approach ensures Whisper has context, without growing indefinitely
      // because the 1000ms VAD pause will finalize and clear it.
      if (isSpeakingRef.current && chunksRef.current.length > 0 && !isSegmentFinalizingRef.current) {
        // Backpressure: Do not dispatch a new interim request if the backend is still processing the previous one.
        // This completely eliminates concurrent processing queue buildup on the CPU!
        if (isTranscribingInterimRef.current) return;
        
        const blob = new Blob(chunksRef.current, { type: mimeType });
        sendAudioToBackend(blob, false, segmentIdRef.current, sttSessionIdRef.current);
      }
    };
